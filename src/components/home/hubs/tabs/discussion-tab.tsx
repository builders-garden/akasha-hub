...
export type DiscussionTabProps = {
  hub: HubDto;
};

export function DiscussionTab({ hub }: DiscussionTabProps): React.JSX.Element {
  return (
    ...
    >
      <div className="flex h-16 flex-row items-center justify-between gap-2 px-6">
        <h1 className="text-sm font-semibold">Discussion for {hub.name}</h1>
      </div>
    </ResponsiveScrollArea>
  );
}
